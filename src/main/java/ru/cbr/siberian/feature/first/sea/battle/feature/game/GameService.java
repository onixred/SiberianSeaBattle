 ...
 */
package ru.cbr.siberian.feature.first.sea.battle.feature.game;

import lombok.RequiredArgsConstructor;
import lombok.extern.slf4j.Slf4j;
import org.springframework.lang.Nullable;
import org.springframework.stereotype.Service;
import ru.cbr.siberian.sea.battle.model.game.*;
...
public class GameService {
    ...
    public int[][] toGridsForOwner(GridPoint[][] grids) {
        return gameMapper.toGridsForOwner(grids);
    }

    public int[][] getGrids(@Nullable Fleet fleet, int sizeGrid, boolean isOwner) {
        if (fleet == null) {
            var grid = new int[sizeGrid][sizeGrid];
            ru.cbr.siberian.sea.battle.service.GameService.initEmptyGrids(grid);
            return grid;
        }
        if (isOwner) {
    ...
    public int[][] toGridsForOpponent(GridPoint[][] grids) {
        return gameMapper.toGridsForOpponent(grids);
    }
}
